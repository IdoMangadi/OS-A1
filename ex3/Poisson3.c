...
int poisson(int argc, char** argv){
    ...
    if (!isNumber(argv[1]) || !isNumber(argv[2]))
    {
     ...
     double long lambda = strtold(argv[1],NULL);
    if (lambda <=0)
    {
    ...
    double long res = pow(lambda, k) * expf(-lambda) / tgamma(k+1); //the tgamma function is the gamma function, notice that the gamma function is defined as gamma(n) = (n-1)! for natural n>0
